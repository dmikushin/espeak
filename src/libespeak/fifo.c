...
static int32_t my_command_is_running = 0;
static int32_t my_stop_is_required = 0;
// + fifo
//
...
static void init(int32_t process_parameters);
...
espeak_ERROR fifo_stop ()
{
  ...
  int32_t a_command_is_running = 0;
  int32_t a_status = pthread_mutex_lock(&my_mutex);
...
{
  //  ENTER("isSpeaking");
  //  int32_t aResult = (int) (my_command_is_running || WaveIsPlaying());
...
static int32_t sleep_until_start_request_or_inactivity()
{
  SHOW_TIME("fifo > sleep_until_start_request_or_inactivity > ENTER");
  int32_t a_start_is_required=0;
...
static void close_stream()
{
  ...
  int32_t a_status = pthread_mutex_lock(&my_mutex);
  assert (!a_status);
  int32_t a_stop_is_required = my_stop_is_required;
  ...
  if (!a_stop_is_required)
      ...
      if (a_stop_is_required)
	{
	  ...
	  int32_t a_status = sem_post(&my_sem_stop_is_acknowledged);
	  assert( a_status != -1);
	}
    }
